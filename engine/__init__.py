"""Main engine of NG-PCE, including pretty much everything and the mainloop.
The engine file has been forked directly from PyCAPGE and has not really been modified, because it is pretty messy.
If you feel like you want to have a boring day, you can try to refactor it like like I wanted to do.
It would be a great enhancement for the project, as I am not a huge fan of having an infinite number of globals everywhere."""
import pygame
import sys
import os
import math
...
# Ignore warning from pygame
# warnings.filterwarnings("ignore", category=UserWarning, module="pygame")

# ==========================================
#  CARGA DEL DICCIONARIO GLOBAL
# ==========================================
def load_translations(filename=cfg.DEFAULT_LANG_FILE):
...
def execute_hotspot_action(hotspot, verb):
    ...
    if verb == "PULL":
        CURRENT_ACTION_ANIM = "pull"
    elif verb == "PICK UP":
        CURRENT_ACTION_ANIM = "pick"
    elif verb == "GIVE":
        CURRENT_ACTION_ANIM = "give"
    elif verb in ["PUSH", "OPEN", "CLOSE", "USE"]:
        CURRENT_ACTION_ANIM = "push"
    ...
    if verb == "PICK UP" and hasattr(hotspot, 'flag_name') and hotspot.flag_name:
        ...
        inventory.add_item(
            hotspot.name,
            hotspot.label,
            hotspot.image_file if hasattr(hotspot, 'image_file') else "default_obj.png",
            # Protección si Ambient no tiene image_file
            hotspot.actions,
            label_id=label_key
        )
        ...
        if isinstance(hotspot, pygame.sprite.Sprite):
            hotspot.kill()
        elif hasattr(scene_manager.current_scene,
                     'ambient_anims') and hotspot in scene_manager.current_scene.ambient_anims:
    ...
    if screen_my < cfg.GAME_AREA_HEIGHT:
        ...
        # ================================================================
            ...
            if inventory.active_item and verbo_efectivo in ["USE", "GIVE"] and not is_right_click:
                ...
                dest_x, dest_y = (hovered_hs.walk_to if hovered_hs.walk_to else (
                hovered_hs.rect.centerx, hovered_hs.rect.bottom + 10))
                smart_move_to(dest_x, dest_y, callback=do_combination)

            else:
                # Clic normal sobre hotspot
                verb = hovered_hs.primary_verb if is_right_click else (
                    sel_verb if sel_verb else hovered_hs.primary_verb)
                dest_x, dest_y = hovered_hs.rect.centerx, hovered_hs.rect.bottom
                if hovered_hs.walk_to:
                    dest_x, dest_y = hovered_hs.walk_to
                elif verb not in ["LOOK AT", "TALK TO"]:
                    dest_y += 10
                else:
                    dest_y += 40
...
def draw_dialogue_mode(screen):
    ...
    pygame.draw.rect(screen, (85, 85, 68), (
    0, cfg.CONFIG["GAME_HEIGHT"] - cfg.CONFIG["BOTTOM_MARGIN"], cfg.CONFIG["GAME_WIDTH"], cfg.CONFIG["BOTTOM_MARGIN"]))
...
def draw_explore_mode(screen):
    ...
    global CURRENT_CURSOR_STATE
    if sel_verb:
        CURRENT_CURSOR_STATE = sel_verb
    elif hovered_hs:
        CURRENT_CURSOR_STATE = hovered_hs.primary_verb
    elif h_item:
        CURRENT_CURSOR_STATE = "LOOK AT"
    elif hovered_exit:
    ...
    highlight = None
    if hovered_hs and not sel_verb:
        highlight = hovered_hs.primary_verb
    elif h_item and not sel_verb:
        highlight = "LOOK AT"
    elif hovered_exit and not sel_verb:
        highlight = "WALK"
    ...
    if CURRENT_STATE != GameState.CUTSCENE:
        ...
        pygame.draw.rect(screen, (85, 85, 68), (
        0, cfg.CONFIG["GAME_HEIGHT"] - cfg.CONFIG["BOTTOM_MARGIN"], cfg.CONFIG["GAME_WIDTH"],
        cfg.CONFIG["BOTTOM_MARGIN"]))
    else:
    ...
    for rect, text, color in elements:
        ...
        if screen_rect.colliderect(draw_rect):
            ...
            if is_exit:
                ...
                if draw_rect.height > 200:
                    label_y = draw_rect.centery
                else:
...
def mainloop():
    ...
    while running:
            ...
            if CURRENT_STATE == GameState.TITLE:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        dialogue_system.abort_dialogue()
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_F1:
                        ...
                        cfg.CONFIG["SHOW_WALKABLE_MASK"] = not cfg.CONFIG["SHOW_WALKABLE_MASK"]
        ...
        if CURRENT_STATE == GameState.MAP:
            ...
            if current_scene:
                ...
                player.update(dt, is_moving=movement.is_moving, direction_x=movement.dir_x, direction_y=movement.dir_y,
                              is_talking=should_player_talk, forced_anim=CURRENT_ACTION_ANIM,
                              current_scene_ref=current_scene)
        ...
        if CURRENT_STATE != GameState.CUTSCENE and not hay_transicion:
            ...
            elif CURRENT_STATE in [GameState.TITLE, GameState.SAVELOAD, GameState.LANGUAGE, GameState.MAP,
                                   GameState.DIALOGUE]:
